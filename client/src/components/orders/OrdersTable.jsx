...
const OrdersTable = () => {
	...
	const handleSearch = (e) => {
		...
		const filtered = filteredTransactions.filter(
			(transaction) => transaction._id.toLowerCase().includes(term) || transaction.customer.toLowerCase().includes(term)
		);
	...
	useEffect(() => {
		(async () => {
			const response = await axios.get(`http://localhost:4000/api/expense/getExpense`, {
	...
	return (
		...
		>
				...
				<table className='min-w-full divide-y divide-gray-700'>
					<thead>
						<tr>
							...
							<th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider'>SubHead</th>
			...
			<div className="flex justify-between items-center mt-4">
				...
				<div className="text-gray-400">Page {currentPage} of {totalPages}</div>
				<button
